...
本应用采用**主备双模型架构**,主模型用阿里云 Qwen-Turbo,备用模型用 DeepSeek-Chat。

| 项目 | 主模型:阿里云 Qwen-Turbo | 备模型:DeepSeek-Chat |
|---|---|---|
...
| 预估月成本(家庭场景) | **< ¥0.05**(详见第 6 节) | **几乎为 0**(只在主模型挂掉时才走 fallback) |
| 模型 ID(代码已写死,仅供参考) | `qwen-turbo` | `deepseek-chat` |
...
### Qwen-Turbo 当前定价(2026-05 阿里云百炼)

- **输入**: ¥0.0003 / 千 tokens
- **输出**: ¥0.0006 / 千 tokens
- **新人免费额度**: 7000 万 tokens / 90 天

### 单次调用规模

一次"资产体检"建议生成,典型 token 用量:

- 输入(脱敏后的资产快照 + system prompt + 模板):约 **400 tokens**
- 输出(润色后的建议文案,3-5 条):约 **300 tokens**
- **单次成本**: 400 × 0.0003/1000 + 300 × 0.0006/1000 = **¥0.00030**(约 0.03 分)

### 月度 / 年度估算

- 家庭场景使用频率:每月录入/调整资产约 5-10 次,每次触发 1-2 次体检,**月最多约 20 次调用**
- **月成本**: 20 × ¥0.00030 ≈ **¥0.006**(不到一分钱)
- **年成本**: ≈ **¥0.07**(七分钱)
- 7000 万免费 token 够你跑大约 **10 万次体检**,等于免费用一辈子
...
**结论:全年总开销 < ¥1**。即使最坏情况(Qwen 免费额度用完 + 频繁触发 fallback),也撑死不超过 ¥5 / 年。
