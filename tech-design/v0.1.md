...
| ORM | Spring Data JPA(写) + JdbcTemplate(读) | MyBatis、jOOQ |
...
| DB | MySQL 8.0 + Flyway 10 | PostgreSQL、SQLite |
...
| 邮件 | spring-boot-starter-mail | 自建 SMTP |
| 部署 | jar(`app.jar`)+ systemd + 已存在的 nginx + 已存在的 TLS | Docker、K8s、消息队列、Redis |
...
│   ├── AsyncConfig.java                   # @EnableAsync · TaskExecutor("metrics","fx","mail")
...
│
├── repository/                            # Spring Data JPA Repository(纯 CRUD)
│   ├── FamilyRepository.java
│   ├── MemberRepository.java
│   ├── AccountRepository.java
│   ├── ... (一表一 repo)
│   └── jdbc/                              # JdbcTemplate-based 复杂只读查询
│       ├── FactQueryDao.java              # ★ FactView 核心查询入口
│       ├── DashboardQueryDao.java
│       └── ReportQueryDao.java
│
...
│   ├── upload/  LogoUploadService.java        # ★ 文件管道(详见 § 7)
...
> 这一节存在的意义:**DDL 改不起**。本节定型即是 v0.1 的"宪法",任何修改触发 Flyway 新版本号。所有 ENUM 用字符串列 + CHECK,**避免 MySQL 原生 ENUM**(增删枚举值要 ALTER TABLE,可移植性差)。
...
```
V<major>__<topic>.sql
  major = 1, 2, 3, ...(单调递增,不留空号)
  topic = 简洁英文,大写开头 OK,蛇形连接

V1__init.sql                  -- §3.1 全部 CREATE TABLE
...
1. 已发布版本号**永不修改**;Flyway `validate-on-migrate=true`。
2. 数据修复 / 一次性脚本走 `R__<name>.sql`(repeatable)或 `V<n>__data_<topic>.sql`。
3. 索引、CHECK、视图都和创建表分开 V 版本号写,便于回顾性对比 PR。

### 3.4 派生 fact view 的实现选择

#### 决策

**v0.1 用 Java 投影 + JdbcTemplate**(在 `repository.jdbc.FactQueryDao` 内封装),**不建 SQL VIEW、不物化**。所有指标只走这个入口。

#### 理由

1. **数据量小**:全家全年 < 200 行 snapshot,内存里 SUM/GROUP 毋须索引技巧。
2. **演化便利**:fact 行需要的派生字段(`account_class`、`account_liquidity`、`_orig` × `FX → _base`)用 Java 表达比 SQL CASE WHEN 清晰,IDE 可重构。
3. **测试便利**:`FactQueryDao` 用 Testcontainers 起一个真实 MySQL 测,投影逻辑用纯函数 `FactProjector.from(rows)` 单测。
4. **未来可平滑升级**:数据增长后,把投影逻辑下推到 SQL `CREATE VIEW account_period_fact AS ...`,`FactQueryDao` 改为直接 `SELECT FROM v_fact` 即可,上层无感。
...
```java
// repository/jdbc/FactQueryDao.java
@Repository
public class FactQueryDao {

    private final NamedParameterJdbcTemplate jdbc;
    public FactQueryDao(NamedParameterJdbcTemplate jdbc) { this.jdbc = jdbc; }

    /** 拉取一段时间窗口内、按筛选条件下的全部 fact 行 */
    public List<AccountPeriodFact> query(FactFilter f) {
        // 1) 拉 base 数据:每个 (account, period) 一行的左外连接
        String sql = """
            SELECT
                a.id            AS account_id,
            ...
            LEFT JOIN fx_rate   fx       ON fx.family_id = a.family_id
                                        AND fx.base_currency  = :baseCurrency
            ...
            WHERE a.family_id   = :familyId
              ...
              AND (:includeArchived = 1 OR a.archived_at IS NULL)
              AND (COALESCE(:accountIdsCsv,'') = '' OR FIND_IN_SET(a.id, :accountIdsCsv))
            GROUP BY a.id, p.id, ps.end_balance, fx.rate
            ORDER BY p.period_start, a.display_order
        """;
        var params = new MapSqlParameterSource()
            .addValue("familyId",      f.familyId())
            .addValue("periodType",    f.periodType().name())
            .addValue("baseCurrency",  f.viewCurrency().code())
            .addValue("rangeStart",    f.rangeStart())
            .addValue("rangeEnd",      f.rangeEnd())
            .addValue("includeArchived", f.includeArchived() ? 1 : 0)
            .addValue("accountIdsCsv", f.accountIds() == null ? "" :
                f.accountIds().stream().map(String::valueOf).collect(Collectors.joining(",")));
        return jdbc.query(sql, params, FactProjector.ROW_MAPPER);
    }
}
```
...
public final class FactProjector {
    ...
    public static final RowMapper<AccountPeriodFact> ROW_MAPPER = (rs, i) -> {
        var t = AccountType.valueOf(rs.getString("account_type"));
        var endOrig = rs.getBigDecimal("end_balance");           // 可能为 null
        var fx      = rs.getBigDecimal("fx_to_base");            // 可能为 null(同币)
        if (fx == null) fx = BigDecimal.ONE;
        return new AccountPeriodFact(
            ...
            Currency.of(rs.getString("account_currency")),
            ...
            rs.getBigDecimal("income_orig"),
            rs.getBigDecimal("expense_orig"),
            rs.getBigDecimal("transfer_in_orig"),
            rs.getBigDecimal("transfer_out_orig"),
            fx
        );
    };
...
**所有 Dashboard / Reports / 任何"能在 fact view 上表达"的查询,都强制经过 `FactViewService`**。Controller 不允许直接调 `FactQueryDao`。
...
2. 但部分场景(如发邮件)**不应**包在事务内 — Spring `@Transactional` 默认 `REQUIRED` 会让邮件失败也回滚业务。
3. 最佳实践是 **service 层标 `@Transactional`,Controller 层不标**;邮件、外部 HTTP 调用放在 `@TransactionalEventListener(AFTER_COMMIT)` 里,事务外执行。
...
```sql
-- snapshotRepo.upsertEndBalance 对应的 SQL(JdbcTemplate)
...
| 属性 | 设置 | 理由 |
|---|---|---|
| `Secure` | true(生产) | 仅 HTTPS 传送;由 `server.servlet.session.cookie.secure=true` 控制 |
...
管理员 A 在 /admin/members
  ...
  → 若启用 SMTP,同时邮件成员 B(包含 tempPwd 的 链接 + 强制改密页)
成员 B 下次登录:
  → 用 tempPwd 登录成功
...
> 这一节存在的意义:文件上传是**最容易出 RCE / 资源耗尽 / Path Traversal** 的地方,不能依赖框架默认安全。
...
[ multipart/form-data 请求 ]
       │
       ▼
1. Spring `MaxUploadSize` 拦截:max-file-size=5MB / max-request-size=6MB
       │
       ▼
2. PathSafety.sanitizeUploadName(originalName)
       │   去掉 ../  \\  ;  &  ` 等;只保留 [a-zA-Z0-9._-];生成新名 = "logo-" + UUID
       ▼
3. Apache Tika MIME 嗅探(读 magic bytes,不信 Content-Type)
       │   允许:image/jpeg, image/png, image/webp, image/svg+xml
       ▼
4. SVG 分支:
       │   · 用 jsoup Whitelist.basic() + 移除 <script> <foreignObject> <use href>;
       │   · 直接落盘(不缩放、不转码);
       │   ✓ 结束
       ▼
5. 光栅分支:
       │
       ├ a. ImageIO.read(InputStream) → BufferedImage
       │      若 width × height > 4096×4096 → reject(防解压炸弹)
       │
       ├ b. imgscalr `Scalr.resize(img, Method.QUALITY, Mode.AUTOMATIC, 256)`
       │
       ├ c. WebP 编码(详见 7.2 推荐 + 备选)
       │
       ├ d. 计算 SHA-256 前 8 位 hash;最终文件名 = "logo-{hash}.webp"(用于 cache busting)
       │
       └ e. 写到 /var/finance/uploads/family-{family_id}/logo-{hash}.webp
              数据库 family.logo_path = "family-{family_id}/logo-{hash}.webp"
              旧文件物理删除
```
...
**首选** `com.sksamuel.scrimage:scrimage-webp:4.1.3`(Java/Scala 通用,内置 libwebp 二进制 fallback,主流 Linux/macOS 开箱即用)。

#### 理由

1. 维护活跃(2025 仍发版),无须自行装系统级 `cwebp`。
2. 提供纯 Java 入口 `WebpWriter.DEFAULT.write(ImmutableImage img, Path)`。
3. 与 imgscalr 配合自然(后者输出 `BufferedImage`,scrimage 可包装)。

#### 备选(各列优劣)

| 库 | 优 | 劣 |
|---|---|---|
| `com.luciad:webp-imageio-core:0.1.3` | ImageIO SPI,零代码改动 | 上次发布 2014;Apple Silicon 无原生二进制,启动可能失败 |
| `org.sejda.imageio:webp-imageio:0.1.6` | 跨平台二进制齐全 | API 稍繁琐;只支持有损 |
| 系统级 `cwebp` 命令 | 编码质量最高、参数最灵活 | 引入 ProcessBuilder + 部署依赖,违背"一 jar 解决"目标 |
...
```java
// service/upload/LogoUploadService.java
@Service
public class LogoUploadService {
    private static final long  MAX_BYTES  = 5L * 1024 * 1024;
    private static final int   MAX_WH     = 4096;
    private static final int   TARGET_WH  = 256;
    private static final Set<String> ALLOWED = Set.of(
        "image/jpeg","image/png","image/webp","image/svg+xml");

    private final Path uploadRoot;       // /var/finance/uploads
    private final Tika tika = new Tika();

    public String store(long familyId, MultipartFile file) throws IOException {
        if (file.getSize() > MAX_BYTES) throw new UploadException("文件超过 5MB");

        String mime;
        try (var in = file.getInputStream()) { mime = tika.detect(in, file.getOriginalFilename()); }
        if (!ALLOWED.contains(mime)) throw new UploadException("不支持的文件类型: " + mime);

        Path familyDir = uploadRoot.resolve("family-" + familyId);
        Files.createDirectories(familyDir);

        if ("image/svg+xml".equals(mime)) {
            return storeSvg(familyDir, file);
        }
        return storeRaster(familyDir, file);
    }

    private String storeRaster(Path dir, MultipartFile file) throws IOException {
        BufferedImage src;
        try (var in = file.getInputStream()) { src = ImageIO.read(in); }
        if (src == null) throw new UploadException("无法解码图像");
        if (src.getWidth() > MAX_WH || src.getHeight() > MAX_WH)
            throw new UploadException("图像尺寸超过 4096×4096");

        BufferedImage scaled = Scalr.resize(src, Scalr.Method.QUALITY,
            Scalr.Mode.AUTOMATIC, TARGET_WH, Scalr.OP_ANTIALIAS);

        byte[] webp = WebpWriter.DEFAULT
            .withQ(82)
            .write(ImmutableImage.fromAwt(scaled));

        String hash = sha256Hex(webp).substring(0, 8);
        String name = "logo-" + hash + ".webp";
        Path target = dir.resolve(name);
        Files.write(target, webp, StandardOpenOption.CREATE_NEW);

        cleanupOldLogosExcept(dir, name);
        return dir.getFileName() + "/" + name;     // family-{id}/logo-xxx.webp
    }

    /* SVG 分支用 jsoup whitelist 过滤后落盘,略 */
}
```

### 7.3 存储路径与 Spring ResourceHandler
...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    public void addResourceHandlers(ResourceHandlerRegistry r) {
        // /uploads/family-1/logo-xxx.webp → /var/finance/uploads/family-1/logo-xxx.webp
        r.addResourceHandler("/uploads/**")
            .addResourceLocations("file:" + props.getUploadRoot() + "/")
...
| 风险 | 缓解 |
|---|---|
| Path Traversal | `MultipartFile.getOriginalFilename()` 永不参与磁盘路径;新文件名 = `logo-{sha256前8}.webp`;落盘前 `Path.normalize().startsWith(uploadRoot)` 校验 |
| MIME spoofing | Apache Tika 嗅探 magic bytes,不信 `Content-Type` header |
| 解压炸弹 / 巨像 | 解码后 `width × height > 4096²` 拒绝;multipart `max-file-size=5MB` |
| SVG XSS | jsoup 白名单清洗:剔除 `<script>` `<foreignObject>` `<iframe>` `on*` 属性、外链 `<use href>` |
| 文件名冲突 | hash 命名 + `CREATE_NEW` 选项 + 删旧 |
| 恶意目录权限 | `/var/finance/uploads` 属主 `finance:finance`,mode `750`;nginx/Java 进程统一以该用户运行 |
| 服务进程被打满 | systemd `LimitNPROC=512`、`MemoryMax=1G`;Java `-Xmx512m` |
| ImageIO 漏洞 | JDK 21 已含历史 CVE 修复;同时禁用 native 图像解码器(`-Dcom.twelvemonkeys.image.useNative=false`,如未引入 twelvemonkeys 可忽略) |
...
> 这一节存在的意义:**纯计算 bug 是恒等式杀手**;**集成测试漏轨道则用户用一次就发现**。两层都不能省。

### 9.1 单元测试 — 纯函数与投影

| 类 | 用例数 | 覆盖目标 |
|---|---|---|
| `PnlCalculatorTest` | 12 | 5 种账户类型 × {首期/中期/末期} × {正常/缺失} |
| `XirrCalculatorTest` | 6 | PRD § 5.7 4 个必过 case + 求根失败 case + < 12 期降级 case |
| `TwrCalculatorTest` | 8 | 月度切片 + 年化 + NetWorth ≤ 0 跳过 + N < 12 不年化 |
| `IdentityVerifierTest` | 4 | 通过 / 容差内通过 / 容差外抛错 / null 端点 |
| `EmergencyFundCalculatorTest` | 5 | 12 期数据 / < 6 期估算 / 月支出=0 / 周度折算 |
| `FactProjectorTest` | 8 | 派生 class、liquidity、_orig × FX→_base、null fx、null end_balance |
| `LoanPrefillServiceTest` | 6 | 首期跳过 / 双期预填 / 配付源 / 未配付源 / 提前还款警告 |

### 9.2 集成测试 — Testcontainers MySQL
...
abstract class IntegrationBase {
    ...
    static void props(DynamicPropertyRegistry r) {
        ...
        r.add("spring.flyway.locations",    () -> "classpath:db/migration,classpath:db/test-fixtures");
    }
}
```
...
1. **首次填报全流程**:登录 → setup wizard → 创建账户 → cron 触发 PeriodOpener → 填全部余额 → 提交本期 → 第二个成员同样动作 → period 自动 CLOSED → MetricsRecomputeJob 写 audit
2. **轧差引导分类**:填新余额 → 验证未解释金额 → 添加 INCOME → 未解释归零 → todo DONE
3. **LOAN 智能预填 + 联动调整**:第二期开始 → 验证 prefilled_balance + 草稿 transfer → 用户改余额 → transfer 联动 → 提交 → 主恒等式通过
4. **跨账户转账重复检测**:24h 内同 (period, from, to, amount) 两次提交 → 第二次抛 `DuplicateTransferException` → 加 confirm 标志后通过
5. **周期重开 + 重算**:CLOSED 周期 → 重开(必填理由)→ 改余额 → 全员再提交 → 自动 CLOSED → 审计日志 2 条

### 9.3 测试数据 fixture

> 所有集成测试共享一份"张家完整 12 期数据集",位于 `src/test/resources/db/test-fixtures/F_test_zhang.sql`,Flyway 加载顺序在 `V` 之后。

```sql
-- F_test_zhang.sql · 张家 fixture(12 期 × 11 账户)
INSERT INTO family(id, name, brand_text, base_currency, period_type) VALUES (1, '张家', '账房', 'CNY', 'MONTHLY');
INSERT INTO member(id, family_id, username, password_hash, display_name, role_label) VALUES
  (1, 1, 'zhangwei', '$2a$12$...', '张伟', '丈夫'),
  (2, 1, 'lijing',   '$2a$12$...', '李静', '妻子');
-- 11 个账户 + 12 期 + 满期 snapshot/cash_flow/transfer/fx_rate
-- ……(数据量约 200 行 INSERT,可由 Excel 导出 + sql 模板生成)
```

> **关键**:fixture 不可包含真实人姓名/真实银行卡号外样本。所有数字保证 PRD § 5.16 主恒等式成立。
...
| 整体行覆盖率 | ≥ 70% | JaCoCo |
| `calc/*` 包行覆盖率 | ≥ 95% | 同上 |
| `factview/*` 包行覆盖率 | ≥ 90% | 同上 |
| 控制器 happy path | 100% 至少 1 次 MockMvc 通过 | MockMvc |
| 主恒等式校验 | 必须在每个集成测试 `@AfterEach` 调一次 | 自定义 assert |
| 性能基线 | Dashboard 加载 < 200ms / Reports 桑基 < 500ms(测试容器内) | JUnit `@Tag("perf")` 选跑 |
...
# 3. 启动 Spring Boot(profile=dev,Flyway 自动迁移)
...
server {
    ...
    client_max_body_size 6M;             # 与 Spring multipart 对齐
...
# 5) 失败邮件由 systemd OnFailure= 触发,这里只写 log
...
OnFailure=finance-backup-failure-mail.service
...
| 含 V<n>__xxx.sql 迁移 | ① 触发本周备份 → ② `systemctl stop finance` → ③ 替换 jar → ④ `systemctl start finance`(Flyway 自动迁移)→ ⑤ 监控 log 30s |
