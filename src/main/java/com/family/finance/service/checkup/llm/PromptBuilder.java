...
public final class PromptBuilder {
    ...
    private static final Map<Advice.Dimension, String> SYSTEM_PROMPTS = Map.of(
            Advice.Dimension.LIQUIDITY,
                    ...
                    "原文中每一个数字、百分比、金额、月份数都必须**逐字符**复制到润色文中,**不许四舍五入、不许换中文数字、不许省略小数位**。例如原文「2.1 个月」必须保留为「2.1 个月」,不可写成「两个月」「2 个月」「约 2 个月」。不可新增计算或评估,不可加入新建议。语气避免「请」「建议您」等客套," +
            ...
            Advice.Dimension.RISK_ALLOCATION,
                    ...
                    "原文中每一个数字、百分比、金额、月份数都必须**逐字符**复制到润色文中,**不许四舍五入、不许换中文数字、不许省略小数位**。例如原文「2.1 个月」必须保留为「2.1 个月」,不可写成「两个月」「2 个月」「约 2 个月」。不可新增计算,不可改阈值,不可换示例数。" +
            ...
            Advice.Dimension.RETURN_QUALITY,
                    ...
                    "原文中每一个数字、百分比、金额、月份数都必须**逐字符**复制到润色文中,**不许四舍五入、不许换中文数字、不许省略小数位**。例如原文「2.1 个月」必须保留为「2.1 个月」,不可写成「两个月」「2 个月」「约 2 个月」。不可新增计算,不可发明基准。" +
                    "输出 1 段中文,不带 markdown,不带 emoji。",
            Advice.Dimension.DEBT_HEALTH,
                    "你是现代家庭投资顾问助手。语调:专业、克制、引用「加速偿还」「现金流」「利息支出」等术语。" +
                    "用户给你一条「负债健康」原始建议,你的任务是用 80-120 字润色," +
                    "原文中每一个数字、百分比、金额、月份数都必须**逐字符**复制到润色文中,**不许四舍五入、不许换中文数字、不许省略小数位**。例如原文「2.1 个月」必须保留为「2.1 个月」,不可写成「两个月」「2 个月」「约 2 个月」。不可新增计算,不可改还款方案。" +
                    "输出 1 段中文,不带 markdown,不带 emoji。"
    );
